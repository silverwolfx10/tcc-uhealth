 ...
 *
 */
class RESTController extends \Api\Controllers\BaseController
{
    ...
    /**
     * Set when there is an 'page' query parameter
     * @var integer
     */
    protected $page = null;
    ...
    protected function parseSearchParameters($unparsed)
    {
        ...
        $splitFields = explode(';', urldecode($unparsed));
        $mapped = array();
    ...
    /**
     * Verifica se o parâmetro enviado na URL é JSON
     *
     * @param string parâmetro enviado na URL
     * @return boolean
     * @author 123i Team
     **/
    protected function verifyJsonRequest($param)
    {
        if (json_encode($param)) {
            return true;
        }
        return false;
    }
    ...
    /**
     * Parseamos os parâmetros enviados como JSON
     *
     * @param string $json
     * @author 123i Team
     **/
    protected function parseFromJson($request, $json)
    {
        $params = json_decode($json, true);
        //var_dump($params);die;

        $this->limit = (isset($params['limit']) && $params['limit']) ? $params['limit'] : $this->limit;
        $this->offset = (isset($params['offset']) && $params['offset']) ? $params['offset'] : $this->offset;
        $this->page = (isset($params['page']) && $params['page']) ? $params['page'] : $this->page;
        $this->order = (isset($params['order']) && $params['order']) ? $params['order'] : $this->order;

        $searchParams = (isset($params['q']) && $params['q']) ? $params['q'] : $this->searchFields;
        $fields = (isset($params['fields']) && $params['fields']) ? $params['fields'] : null;

        if ($searchParams) {
            $this->isSearch = true;
            $this->searchFields = $searchParams;
     ...
     * @author 123i Team
     **/
    protected function parseFromDefaultFormat($request)
    {
        ...
        $this->order = ($request->get('order', null, null)) ?: $this->order;

        if (!is_null($this->order)) {
            $ordersToParse = $this->parsePartialFields($this->order);
            $this->order = [];
            if (is_array($ordersToParse)) {
                foreach ($ordersToParse as $key => $order) {
                    // $this->order = array_merge($this->parseSearchParameters($order), $this->order);
                    // array_push($this->order, $this->parseSearchParameters($order));
                    $this->order = $this->order + $this->parseSearchParameters($order);
                }

            } else {
                $this->order = $this->parseSearchParameters($ordersToParse);
            }
        }
    ...
    protected function parseRequest($allowedFields)
    {
        ...
        $jsonParams = $request->get('json', null, null);

        if (!is_null($jsonParams)) {

            $this->parseFromJson($request, $jsonParams);
        } else {
            $this->parseFromDefaultFormat($request);
        }
